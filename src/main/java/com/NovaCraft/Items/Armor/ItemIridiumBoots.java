...
public class ItemIridiumBoots extends ItemArmor implements ISpecialArmor
{
	AttributeModifier health_increase_overworld = new AttributeModifier("generic.maxHealth", 1.0D, 0);
	
    public ItemIridiumBoots() {
        super(NCArmorMaterial.IRIDIUM, 0, 3);
        this.setCreativeTab(NovaCraftCreativeTabs.tools);
    }
    
    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String type) {
        if (stack.getItem() == NovaCraftItems.iridium_boots) {
            return "nova_craft:textures/armor/iridium_layer_1.png";
        }
        FMLLog.log(Level.ERROR, "Invaild Texture for Iridium Boots", new Object[0]);
        return null;
    }
	...
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
    	ItemIridiumBoots armorItem = (ItemIridiumBoots) armor.getItem();
