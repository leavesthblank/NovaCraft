...
public class ItemPotionInvisibilityExtended extends ItemNovaCraftFood {
	...
	public void registerIcons(final IIconRegister iconRegister) {
		this.textures = new IIcon[ItemPotionInvisibilityExtended.types.length];
		for (int i = 0; i < ItemPotionInvisibilityExtended.types.length; ++i) {
			this.textures[i] = iconRegister.registerIcon("nova_craft:" + ItemPotionInvisibilityExtended.types[i]);
		}
	}
	...
	public String getUnlocalizedName(final ItemStack itemstack) {
		...
		if (meta < 0 || meta >= ItemPotionInvisibilityExtended.types.length) {
			meta = 0;
		}
		return super.getUnlocalizedName() + "." + ItemPotionInvisibilityExtended.types[meta];
	}

	public void getSubItems(final Item item, final CreativeTabs creativeTabs, final List list) {
		for (int meta = 0; meta < ItemPotionInvisibilityExtended.types.length; ++meta) {
			list.add(new ItemStack(item, 1, meta));
		}
	}

	static {
		types = new String[] { "potion_invisibility", "potion_invisibility_regular" };
	}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List tooltip, final boolean who) {
		int meta = stack.getItemDamage();
		if (meta == 0) {
			tooltip.add(EnumChatFormatting.GRAY + "" + StatCollector.translateToLocal("tooltip.potion.invis_extend.recipe.desc"));
			tooltip.add(EnumChatFormatting.GRAY + "" + StatCollector.translateToLocal("tooltip.potion.invisibility.desc"));
		} else if (meta == 1) {
			tooltip.add(EnumChatFormatting.GRAY + "" + StatCollector.translateToLocal("tooltip.potion.invisibility.regular.desc"));
		}
    }
