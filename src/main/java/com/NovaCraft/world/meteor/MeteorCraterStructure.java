...
public class MeteorCraterStructure extends WorldGenerator {
    ...
    private void generateCrater(World world, Random rand, int x, int y, int z) {
        if (y <= 85) {
            ...
            int radiusInt = MathHelper.floor_float(radius + 6.0F);
            ...
            int maxY = Math.min(world.getActualHeight(), y + 32);
            ...
            for (checkY = -radiusInt; checkY <= radiusInt; ++checkY) {
                for (dz = -radiusInt; dz <= radiusInt; ++dz) {
                    ...
                    if (horizDistSq <= (double)(radius * radius)) {
                        ...
                        for (int realY2 = y + 1; realY2 < maxY; ++realY2) {
                            if (!world.blockExists(realX, realY2, realZ)) continue;

                            if (!world.isAirBlock(realX, realY2, realZ)) {
                                world.setBlockToAir(realX, realY2, realZ);
                            }
                        }
                    }
                }
        ...
        for(int dx = -radiusInt; dx <= radiusInt; ++dx) {
            for(int dz = -radiusInt; dz <= radiusInt; ++dz) {
                ...
                if (horizDistSq <= (double)(radius * radius)) {
                    ...
                    if (floorY > 0) {
                        if (!world.blockExists(realX, floorY, realZ)) continue;
                        ...
                        if (!world.blockExists(realX, floorY + 1, realZ)) continue;
                        ...
                        if (world.isAirBlock(realX, floorY + 1, realZ) && target.getMaterial().isSolid() && target != Blocks.log && target != Blocks.log2 && target != Blocks.leaves && target != Blocks.leaves2) {
                            world.setBlock(realX, floorY + 1, realZ, bottomBlock, 0, 2);
                        }
                    }
                }
            }
            ...
            for(int dz = -radiusInt; dz <= radiusInt; ++dz) {
                ...
                if (horizDistSq <= (double)(radius * radius)) {
                    ...
                    if (floorY > 0) {
                        ...
                        float chance = rand.nextFloat();
                        Block bottomBlock = chance < 0.85F ? Blocks.sand : Blocks.dirt;
                        Block target = world.getBlock(realX, floorY, realZ);
                        if (world.isAirBlock(realX, floorY + 1, realZ) && target.getMaterial().isSolid() && target != Blocks.log && target != Blocks.log2 && target != Blocks.leaves && target != Blocks.leaves2) {
                            world.setBlock(realX, floorY + 1, realZ, (Block)bottomBlock);
                        }
                    }
                }
            }
    ...
    private void decorateCrater(World world, Random rand, int x, int y, int z, int radiusInt, float radius) {
        for (int extraRadius = -radiusInt; extraRadius <= radiusInt; ++extraRadius) {
            for (int dz = -radiusInt; dz <= radiusInt; ++dz) {
                for (int dx = -24; dx <= 0; ++dx) {
                    int realX = x + extraRadius;
                    int realY = y + dx;
                    int realZ = z + dz;
                    double distSq = (double)(extraRadius * extraRadius + dz * dz) + (double)(dx * dx) * 4.5D;

                    if (Math.abs(distSq - (double)(radius * radius)) < 6.0D && rand.nextFloat() < 0.08F) {
                        if (world.blockExists(realX, realY, realZ)) {
        ...
        int extraRadius2 = radiusInt + 6;
        int depth = 8;

        for (int dx = -extraRadius2; dx <= extraRadius2; ++dx) {
            for (int dz = -extraRadius2; dz <= extraRadius2; ++dz) {
                double horizDistSq = (double)(dx * dx + dz * dz);
                if (horizDistSq <= (double)(extraRadius2 * extraRadius2)) {
                    int realX = x + dx;
                    int realZ = z + dz;

                    for (int dy = -depth; dy <= 0; ++dy) {
                        int realY = y + dy;
                        if (realY >= 5 && rand.nextFloat() < 0.04F && world.blockExists(realX, realY, realZ)) {
                            this.tryPlaceOre(world, rand, realX, realY, realZ);
                        }
                    }
                }
    ...
    private Block pickRandomOre(Random rand) {
        ...
        } else if (chance >= 0.9F && chance < 0.94F) {
            return Blocks.gold_ore;
        } else if (chance >= 0.94F && chance <= 0.95F) {
            return Blocks.diamond_ore;
        } else {
            return chance > 0.95F && chance <= 0.98F ? NovaCraftBlocks.iridium_ore : NovaCraftBlocks.meteorite_block;
        }
    }
        ...
        for (int dx = -radiusInt; dx <= radiusInt; ++dx) {
            for (int dz = -radiusInt; dz <= radiusInt; ++dz) {
                ...
                if (horizDistSq <= (double)(radius * radius)) {
                    ...
                    for (int dy = -24; dy <= 1; ++dy) {
                        int realY = y + dy;
                        if (realY >= 0 && world.blockExists(realX, realY, realZ)) {
                            Block block = world.getBlock(realX, realY, realZ);
                            if (block.getMaterial().isLiquid()) {
                                world.setBlockToAir(realX, realY, realZ);
                            }
