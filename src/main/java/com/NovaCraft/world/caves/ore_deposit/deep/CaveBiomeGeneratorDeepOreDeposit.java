...
public class CaveBiomeGeneratorDeepOreDeposit extends MultiChunkFeatureGeneratorDeepOreDeposit {

    public final UndergroundBiomeDeepOreDeposit biome;
    ...
    private static final int BLOBS_CELL_SIZE = 1;     // bigger = thicker, larger patches
    private static final double NOISE_AMPLITUDE = 0.9999; // smaller = more consistent coverage
    private static final double SURFACE_SHELL = 0.0001; // bigger = more blocks qualify near surface
    ...
    public CaveBiomeGeneratorDeepOreDeposit(UndergroundBiomeDeepOreDeposit biome, int rarity, int minXSize, int minYSize, int minZSize,
    ...
    public void generateChunkPart(BlockPos src, Random random, int chunkX, int chunkZ, World world) {
        ...
        for (int i = 0; i < 1 + random.nextInt(1); i++) {
            int offsetX = src.x + random.nextInt(10) - 5;
            int offsetY = src.y + random.nextInt(10) - 5;
            int offsetZ = src.z + random.nextInt(10) - 5;
    ...
    public void apply(World world, int centerX, int centerY, int centerZ,
        ...
        CaveBiomeGeneratorDeepOreDeposit.UndergroundBiomeGenerationContext context = new CaveBiomeGeneratorDeepOreDeposit.UndergroundBiomeGenerationContext();
        ...
        forEachChunkBlock(chunkX, chunkZ, centerY - radiusY, centerY + radiusY, (pos) -> {
            ...
            if (APPLY_Y_CULL) {
                if (pos.y > 24) {
                    if (pos.y > 28 || random.nextFloat() > 0.33f) return;
                }
            }
    ...
    public static class UndergroundBiomeData {
        public UndergroundBiomeData(UndergroundBiomeDeepOreDeposit biome, int rarity,
