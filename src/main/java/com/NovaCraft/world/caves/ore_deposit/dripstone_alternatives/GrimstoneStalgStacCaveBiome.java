...
import com.NovaCraft.world.caves.ore_deposit.deep.BasicUndergroundBiomeDeepOreDeposit;
...
public class GrimstoneStalgStacCaveBiome extends BasicUndergroundBiomeDeepOreDeposit {

    public static boolean useOre;
    public static double StagChance;
    public static double StacChance;

    public GrimstoneStalgStacCaveBiome() {
        super(NovaCraftBlocks.grimstone, null, null, 0, 0, 0, true);
    }

    private static boolean canReplace(Block block) {
        if (block == null) return false;
        String name = block.getUnlocalizedName();
        return name != null && (name.toLowerCase(Locale.ROOT).contains("grim")) && !(name.toLowerCase(Locale.ROOT).contains("ore")) && !(name.toLowerCase(Locale.ROOT).contains("dirt"))
                && !(name.toLowerCase(Locale.ROOT).contains("brick"));
    }
    ...
    public void fillCeiling(World world, int x, int y, int z, Block block) {
        ...
        world.setBlock(x, y, z, NovaCraftBlocks.grimstone, 0, 2);
    }
    ...
    public void fillFloor(World world, int x, int y, int z, Block block) {
        ...
        switch (chance / 20) {
            case 0:
                placeBlockBottom = NovaCraftBlocks.massive_nullstone_stalagmite_top;
                break;
            case 1:
                placeBlockBottom = NovaCraftBlocks.large_grimstone_stalagmite;
                break;
            case 2:
                placeBlockBottom = NovaCraftBlocks.calvatia_mushroom;
                break;
            case 3:
                ...
                break;
            case 4:
                placeBlockBottom = NovaCraftBlocks.massive_nullstone_stalagmite_bottom;
                break;
            default:
                placeBlockBottom = NovaCraftBlocks.grimstone_stalagmite;
                break;
        }
        ...
        if (world.getBlock(x, topY, z).isAir(world, x, topY, z) && !canSeeSky(world, x, topY, z)) {
            if (placeBlockBottom == NovaCraftBlocks.massive_nullstone_stalagmite_bottom) {
                if (world.getBlock(x, topY + 1, z).isAir(world, x, topY + 1, z)) {
                    world.setBlock(x, topY, z, NovaCraftBlocks.massive_nullstone_stalagmite_bottom, 0, 2);
                    world.setBlock(x, topY + 1, z, NovaCraftBlocks.massive_nullstone_stalagmite_top, 0, 2);
                }
            } else {
        ...
        if (chance_top / 20 == 0) {
            placeBlockTop = NovaCraftBlocks.grimstone_stalactite;
        } else if (chance_top / 20 == 1) {
            placeBlockTop = NovaCraftBlocks.massive_nullstone_stalactite_top;
        } else {
            placeBlockTop = NovaCraftBlocks.large_grimstone_stalactite;
        }
        ...
        for (int dy = 2; dy <= maxScan; dy++) {
            ...
            if (ceiling != null && ceiling.getMaterial().isSolid() && ceiling.isOpaqueCube()) {
                if (world.getBlock(x, hangY, z).isAir(world, x, hangY, z) && !canSeeSky(world, x, hangY, z)) {
                    if (placeBlockTop == NovaCraftBlocks.massive_nullstone_stalactite_top) {
                        world.setBlock(x, hangY, z, placeBlockTop, 0, 2);
                        world.setBlock(x, hangY - 1, z, NovaCraftBlocks.massive_nullstone_stalactite_bottom, 0, 2);
                    } else {
                        world.setBlock(x, hangY, z, placeBlockTop, 0, 2);
        ...
        Block placeLichen = NovaCraftBlocks.grim_lichen;
