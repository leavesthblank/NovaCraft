...
public class EntityWyrmling extends EntityFlying implements IMob {
    ...
    protected void updateEntityActionState() {
        ...
        if (distanceScaled < 1.0 || distanceScaled > 8100.0) {
        ...
        if (this.targetedEntity == null) {
            this.targetedEntity = (EntityLivingBase)this.getClosestVulnerableVisiblePlayer(60.0);
        }
        if (this.targetedEntity != null) {
    ...
    public void onCollideWithPlayer(final EntityPlayer player) {
        ...
        if (!player.capabilities.isCreativeMode && !this.worldObj.isRemote && this.getEntitySenses().canSee((Entity)player) && this.getDistanceToEntity((Entity)player) <= 1.8f && player.boundingBox.maxY >= this.boundingBox.minY && player.boundingBox.minY <= this.boundingBox.maxY && this.attackTime <= 0 && this.attackEntityAsMob((player))) {
            ...
            this.playSound("nova_craft:deepoid.hurt", 1.0f, 1.0f);
        }
    }
    ...
    public void onEntityUpdate() {
        ...
        if (this.worldObj.isRemote) {
            ...
            if (f > 0.0f && f2 <= 0.0f) {
                this.worldObj.playSound(this.posX, this.posY, this.posZ, "mob.enderdragon.wings", 0.95f + this.rand.nextFloat() * 0.05f, 0.95f + this.rand.nextFloat() * 0.05f, false);
            }
        }
    }
    ...
    public boolean getCanSpawnHere() {
        ...
        return (this.posY >= 95.0D && this.worldObj.canBlockSeeTheSky(i, j, k) && this.worldObj.getBlockLightValue(i, j, k) >= 7 && canSpawn);
    }
    ...
    protected void dropFewItems(final boolean p_70628_1_, final int p_70628_2_) {
        if (p_70628_1_) {
            for (int j = 2 + this.rand.nextInt(3) + p_70628_2_, k = 0; k < j; ++k) {
                this.entityDropItem(new ItemStack(NovaCraftItems.wyrmling_scales), 1.5f);
            }
        }
    }
